...
func updateStatus(r io.Reader) {
	...
	for scanner.Scan() {
		txt := scanner.Text()
		updateBytesReceivedRate(txt)
		updateTotalReceived(txt)
	}
}

func updateBytesReceivedRate(txt string) {
...
func updateTotalReceived(txt string) {
	res := strings.Index(txt, "Cumulative (sent/received/total):")
	if res == -1 {
		return
	}
	sub1 := txt[res:len(txt)]
